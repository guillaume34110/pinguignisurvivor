...
export const creaturesUpdate = (gameData: GameData) => {
    for (let i = 0; i < gameData.creatures.length - 1; i++) {
        ...
        for (let k = 0; k < creature.sensors.sensors.length - 1; k++) {
            const sensor = creature.sensors.sensors[k]
            creature_updateSensorsPosition(creature, sensor)

            for (let ka = 0; ka < gameData.items.length - 1; ka++) {
        ...
        for (let t = 0; t < gameData.items.length - 1; t++) {
        ...
        for (let j = 0; j < gameData.creatures.length - 1; j++) {
        ...
        for (let l = 0; l < gameData.mapBlocks.length - 1; l++) {
